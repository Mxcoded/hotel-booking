<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <!-- Image -->
    <div>
        <label for="image" class="block text-sm font-medium text-gray-700">Room Image @if(request()->routeIs('admin.rooms.create')) (Required) @else (Optional) @endif</label>
        <input type="file" name="image" id="image" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" @if(request()->routeIs('admin.rooms.create')) required @endif>
        @isset($room->image)
...
<div class="mt-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
        @php
            // Get an array of the room's current feature icons for easy checking
            $currentFeatures = isset($room) && is_array($room->features) ? array_column($room->features, 'icon') : [];
        @endphp
        @foreach($features as $iconClass => $name)
            <label class="flex items-center space-x-3">
                <input type="checkbox" name="features[]" value="{{ $iconClass }}"
                       @if(in_array($iconClass, old('features', $currentFeatures))) checked @endif
                       class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                <span><i class="fas {{ $iconClass }} mr-2 text-gray-600"></i>{{ $name }}</span>
            </label>
        @endforeach
    </div>
</div>
